...
mymainmenu = awful.menu({ items = { { "Awesome", myawesomemenu, beautiful.awesome_icon },
      ...
      { "常用 (&U)", mymenu },
      { "应用程序 (&A)", xdgmenu },
      { "挂起 (&S)", "sudo pm-suspend" },
      { "关机 (&H)", "sudo halt", '/usr/share/icons/gnome/16x16/actions/gtk-quit.png' },
      }
})
...
cputempwidget_clock:add_signal("timeout", function()
  ...
  f:close()
  if fc and tonumber(fc:match('%d+')) > 70 then
  naughty.notify({title="警告", text="CPU 温度已超过 70℃！", timeout=5, preset=naughty.config.presets.critical})
  end
...
globalkeys = awful.util.table.join(
  -- 音量 {{{3
  awful.key({ modkey, }, "Up", function () awful.util.spawn("amixer -q sset Master 10%+ u") end),
  awful.key({ modkey, }, "Down", function () awful.util.spawn("amixer -q sset Master 10%- u") end),
