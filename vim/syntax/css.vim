...
syn keyword cssTagName textarea tfoot th thead title tr tt ul u variant
" S HTML5 tags
syn keyword cssTagName article aside audio canvas command datalist details
syn keyword cssTagName figcaption figure footer header hgroup keygen mark
...
" S Rules for CSS3
syn match cssAnimeProp contained "\(-\(moz\|webkit\|ms\|o\)-\|\)animation-\(name\|duration\|iteration-count\|direction\|timing-function\)\>"
syn match cssAnimeProp contained "\(-\(moz\|webkit\|ms\|o\)-\|\)\(transition\|transform\)\>"

syn match cssBoxProp contained "\(-\(moz\|webkit\|ms\|o\)-\|\)box-shadow\>"
syn match cssBoxProp contained "\(-\(moz\|webkit\|ms\|o\)-\|\)border-\(radius\|image\)\>"
syn match cssBoxProp contained "\(-\(moz\|webkit\|ms\|o\)-\|\)border-radius\(-\(bottomleft\|bottomright\|topright\|topleft\)\)\>"
syn match cssBoxProp contained "\(-\(moz\|webkit\|ms\|o\)-\|\)box-sizing\>"
syn match cssBoxProp contained "\(-\(moz\|webkit\|ms\|o\)-\|\)column-\(count\|width\|gap\|rule\)\>"
syn match cssBoxProp contained "\(-\(moz\|webkit\|ms\|o\)-\|\)box-reflect\>"
syn match cssBoxProp contained "\<resize\>"

syn match cssTextAttr contained "\<break-word\>"
syn match cssTextProp contained "\<word-wrap\>"
syn match cssTextProp contained "\(-\(moz\|webkit\|ms\|o\)-\|\)text-\(fill-color\|\(stroke-\(color\|width\)\)\|overflow\)\>"
" E Rules for CSS3
